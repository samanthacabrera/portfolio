import React from "react";

function QandA() {
    return (
       <div className="space-y-20">
            <h2 className="subHeading">Q & A</h2>
            <ul className="space-y-8">
                <li>What is your favorite aspect of software engineering?</li>
                <li>My favorite aspect of software engineering is the structured problem-solving inherent when programming. There's a satisfaction in dissecting complex issues and crafting elegant solutions through code.</li>
                <li>What are some goals or aspirtations you have for your career in software development?</li>
                <li>What motivates you to continue to learn and grow as a software engineer?</li>
            </ul>
        </div>
    )
}

export default QandA